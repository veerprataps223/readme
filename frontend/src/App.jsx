...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (urlParams.get("auth") === "success" && token) {
      saveToken(token);
  ...
  const checkAuthStatus = async (token = null) => {
    try {
      ...
      if (data.authenticated && data.user) {
        setUser(data.user);
        if (data.token && data.token !== tokenToUse) {
        ...
        <div className="output-controls">
          ...
          {analysis && (
            <div className="analysis-summary" aria-live="polite" tabIndex={-1}>
              ...
              {analysis.repository?.private ? "Private" : "Public"} repo
            </div>
          )}
        </div>
      </section>
        ...
        >
          ...
          <div className="analysis-grid">
            ...
            <article className="analysis-card" tabIndex={0}>
              ...
              <h3>Complexity</h3>
              <p>{analysis.projectPurpose || "Simple"}</p>
            </article>
            ...
            <article className="analysis-card" tabIndex={0}>
              ...
              <h3>Stars</h3>
              <p>{analysis.repository?.stars?.toLocaleString() || 0}</p>
            </article>
          </div>
        </section>
      )}
